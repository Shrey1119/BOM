...
def detect_repository_source(component, src_dir):
    ...
    # Retrieve local path properties from Syft
    paths = []
    for p in component.get('properties', []):
        if p.get('name', '').startswith('syft:location:') and p.get('name', '').endswith(':path'):
    ...
    # Fallback heuristic based on syft metadata
    for p in component.get('properties', []):
        ...
        try:
            with open(file_path, 'rb') as f:
                ...
                # Unix ELF headers
                    ...
                    # Check execute permission if on Linux/Mac
                    if os.name == 'posix':
                ...
                # Zip archive / Java archive manifest check
                    ...
                    if ext in ['.jar', '.war']:
                        ...
                        # Look for executable entry point in JAR manifest
                        import zipfile
                        try:
...
def enrich_component(component, config, sbom_context):
    ...
    depth = sbom_context['depths'].get(bom_ref, 2)
