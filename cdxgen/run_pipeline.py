# -*- coding: utf-8 -*-
"""
cdxgen SBOM CLI Pipeline Menu
-------------------------------
Provides an interactive menu for:
  1) Set target source path
  2) Run scanning and verification pipeline (cdxgen + Grype + Enrich + Validate)
...
def banner():
    print("")
    print("=" * 66)
    print("        cdxgen SBOM Compliance Pipeline - CLI Menu")
    print("=" * 66)
...
def action_scan(target_path):
    ...
    try:
        ...
        print("\n  [!] Error: Node.js/npx not found in target path.")
        return False
        
    if not os.path.exists(GRYPE_EXE):
        print("\n  [!] Error: portable grype.exe not found at: {}".format(GRYPE_EXE))
        print("      Please run setup_tools.py in Member 4 folder first.")
        return False

    print("\n  >> Starting cdxgen compliance pipeline scan...\n")
    ...
    # Step 2: cdxgen Scan (10% - 40%)
    step2 = ScanStepProgress("[Phase 2/5] Running cdxgen deep dependency cataloging", 10, 40)
    ...
    try:
        run_process_with_progress(cdxgen_cmd, step2, cwd=src_dir_abs, shell=True)
        ...
        if os.path.exists(temp_raw_path):
            ...
            print("\n  [!] Error: cdxgen completed but output file not found.")
            return False
    except Exception:
        print("\n  [!] cdxgen cataloging phase failed.")
        if os.path.exists(temp_raw_path):
