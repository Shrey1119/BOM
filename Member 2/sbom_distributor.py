...
def canonicalize_json(json_data):
    ...
    # We strip any existing signature or temporary fields to ensure we sign only the contents
    data_copy = json_data.copy()
    data_copy.pop("signature", None)
    
    # Standardize sorting of keys and eliminate whitespace to prevent formatting differences
...
def split_sbom(sbom_data):
    ...
    # Optional: Strip other private configurations or metadata properties if present
    if "metadata" in public_sbom and "properties" in public_sbom["metadata"]:
        # Filter out properties that are internal/private
        filtered_props = [
    ...
    # Private SBOM: Retain everything (vulnerabilities and linkages)
...
def embed_signature(sbom_json, signature_b64, public_key_pem):
    ...
    # 1. Embed under the top-level 'signature' block (standard JSON signature format)
    ...
    # 2. Also embed in metadata properties as key-value pairs for downstream tools
    ...
    # Remove existing signature properties if any
    sbom_signed["metadata"]["properties"] = [
        p for p in sbom_signed["metadata"]["properties"]
        if p.get("name") != "signature"
    ]
    
    # Append the new signature property
...
def main():
    parser = argparse.ArgumentParser(description="SBOM Split & Sign Distributor (Team Member 2)")
    ...
    # Get Public Key PEM for embedding
    ...
    # Verify signatures locally immediately
    ...
    # 5. Embed signatures inside the JSON documents
    ...
    # Write detached signatures (base64)
